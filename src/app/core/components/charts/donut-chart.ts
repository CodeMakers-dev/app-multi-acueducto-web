...
import { Component, OnInit, OnDestroy, ElementRef, ViewChild, ChangeDetectionStrategy, inject, PLATFORM_ID } from '@angular/core';
...
export class DonutChartComponent implements OnDestroy {
            ...
            labels: {
              ...
              total: {
                ...
                formatter: function (w: any): string {
                  const sum: number = w.globals.seriesTotals.reduce((a: number, b: number) => {
                    return a + b;
                  }, 0);
                  return '$' + sum + 'k';
                },
  ...
  handleCheckboxChange(event: Event, chart: any): void {
    ...
    if (checkbox.checked) {
      ...
      const allCheckboxes = document.querySelectorAll('#devices input[type="checkbox"]') as NodeListOf<HTMLInputElement>;
      allCheckboxes.forEach((cb: HTMLInputElement) => {
        if (cb !== checkbox) {
          cb.checked = false;
        }
      });

      // Actualizar series según el valor del checkbox
      switch (checkbox.value) {
        case 'desktop':
          chart.updateSeries(chartData.desktop);
          break;
        case 'tablet':
          chart.updateSeries(chartData.tablet);
          break;
        case 'mobile':
          chart.updateSeries(chartData.mobile);
          break;
        default:
          chart.updateSeries(chartData.default);
      }
    } else {
      // Si se desmarca, volver a los datos por defecto
      chart.updateSeries(chartData.default);
    }
  }
  ...
  initializeDonutChart(): void {
    ...
    if (chartElement && typeof ApexCharts !== 'undefined') {
      ...
      const checkboxes = document.querySelectorAll('#devices input[type="checkbox"]') as NodeListOf<HTMLInputElement>;
      checkboxes.forEach((checkbox: HTMLInputElement) => {
        checkbox.addEventListener('change', (event: Event) => {
          this.handleCheckboxChange(event, this.chart);
        });
      });
    } else {
